...
function sharerDisplayName(userData: Record<string, unknown>, fallbackEmail?: string): string {
  const name = typeof userData.name === 'string' ? userData.name.trim() : '';
  if (name) return name;
  const email = typeof userData.email === 'string' ? userData.email.trim() : fallbackEmail ?? '';
  ...
  try {
    ...
    if (!target) {
      return NextResponse.json(
        {
          ok: false,
          error: 'No hay un usuario registrado con ese email. Revisá el correo o pedile que se registre en LegalMev.',
          code: 'USER_NOT_FOUND',
        },
        { status: 404 },
      );
    ...
    const sharedWithUids = sharedWithUidsFrom(sharedWith);

    await adminDb.collection('controlPrueba').doc(id).update({
      sharedWith,
      sharedWithUids,
    ...
    const emailResult = await sendResourceShareNotification({
      ...
      kind: 'control-prueba',
      resourceId: id,
      resourceTitle: String(owned.data.caratula ?? 'Control de prueba'),
      role,
    });

    const updated = await adminDb.collection('controlPrueba').doc(id).get();
    ...
    return NextResponse.json({
      ...
      expediente: serializeControlPruebaDoc(id, updated.data() ?? owned.data),
...
/** PATCH — cambiar rol de un colaborador (solo dueño). */
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (!targetUid || !isShareRole(body.role)) {
      return NextResponse.json({ ok: false, error: 'uid y role (view|edit) son obligatorios' }, { status: 400 });
    }
...
/** DELETE — revocar acceso (?uid=) (solo dueño). */
