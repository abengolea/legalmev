 ...
 * Webhook IPN de DLocal Go. Cuando el pago está PAID:
  ...
  try {
    ...
    if (orderId.startsWith('colegio-')) {
      ...
      if (colegioId) {
        ...
        console.log('[webhook-dlocal] Pago colegio registrado:', colegioId, periodo);
      }
      return NextResponse.json({ ok: true });
    }
    ...
    if (userSnap.exists && userData?.tier !== 'premium') {
      ...
      console.log('[webhook-dlocal] Usuario', uid, 'actualizado a premium (order:', orderId, ')');
    }
