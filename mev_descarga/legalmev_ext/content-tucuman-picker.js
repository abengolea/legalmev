/**
 * Tucumán SAE — modal de selección + PDF único LegalMev (mismo flujo que MEV/PJN/Salta).
 */
(function () {
  'use strict';
  if (window.__LEGALMEV_TUCUMAN_PICKER__) return;
  window.__LEGALMEV_TUCUMAN_PICKER__ = true;

  function core() {
    return globalThis.LegalMevTucumanCore;
  }
  ...
  async function openDownloadFlow() {
    ...
    const C = core();
    if (!Ui || !C) {
      alert('LegalMev: componentes Tucumán no cargados. Recargá la página (F5).');
      return;
    }
    ...
    const picker = Ui.openPicker({
      portal: 'TUCUMAN',
      title: 'Armar exportación Tucumán',
      subtitle:
        'Seleccioná los movimientos de la Historia. LegalMev arma el PDF con tipografía y colores propios.',
      items: [],
      originLabel: 'Tucumán · Portal SAE',
      async onExport({ mode, selectedItems, setProgress, cancelFlag }) {
        ...
        datos.portal = 'TUCUMAN';
        datos.jurisdiccion = datos.jurisdiccion || 'Tucumán';
    ...
    picker.setLoadingMessage('Obteniendo movimientos del SAE…');
    try {
      ...
      picker.setItems(items, {
        originLabel: `Tucumán · ${items.length} movimiento${items.length === 1 ? '' : 's'}`,
      });
    } catch (e) {
  ...
  chrome.runtime.onMessage.addListener((msg, _s, sendResponse) => {
    if (msg?.type === 'START_DOWNLOAD_PICKER' || msg?.type === 'TUCUMAN_START_ZIP_EXPORT') {
      openDownloadFlow()
